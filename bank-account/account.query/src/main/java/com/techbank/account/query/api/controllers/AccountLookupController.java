...
public class AccountLookupController {
    ...
    @GetMapping(path = "/byType/{accountType}")
    public ResponseEntity<AccountLookupResponse> getAccountById(@PathVariable(value = "accountType") AccountType accountType) {
        try {
            List<BankAccount> accounts = queryDispatcher.send(new FindAccountByTypeQuery(accountType));
            if (accounts == null || accounts.size() == 0) {
                return new ResponseEntity<>(null, HttpStatus.NO_CONTENT);
            }
            var response = AccountLookupResponse.builder()
